...
func GetExcelFileOnArgs(app Applicationer, resp *response.Response, rfltArgs reflect.Value, fieldMD fields.FieldCollection, modelRow ObjectRow, clientFilName string) error {
	...
	query, query_tot, where_params, err := GetListQuery(rfltArgs, fieldMD, modelRow, conn)
	if err != nil {
		return err
	}
	
	model_xml, err := QueryResultToXML(modelRow, query, query_tot, where_params, conn)
	if err != nil {
		return err
	}
...
func QueryResultToXML(scanStruct ObjectRow, query string, queryTotal string, whereParams []interface{}, conn *pgx.Conn) (string, error) {
	//tot
	tot_cnt := 0 
	if queryTotal != "" {
	...
	r_cnt := 0
	for rows.Next() {
		row := reflect.New(reflect.ValueOf(scanStruct).Elem().Type()).Interface().(ObjectRow)
	...
	return fmt.Sprintf(`<model id="%s" totalCount="%d">%s</model>`, scanStruct.GetID(), tot_cnt, xml_rows),
		nil
}
